...
elseif (route(2) == "pages"):
  ...
  if ($access):
    if (route(3) == "edit"):
      ...
      if (!$page): header("Location:" . site_url("admin/appearance/pages"));
      endif;
    elseif (!route(3)):
  ...
  if (route(5)): header("Location:" . site_url("admin/appearance/pages"));
  endif;
...
elseif (route(2) == "blog"):
  ...
  if ($access):
    ...
    if (route(3) == "edit"):
      ...
      if (!$post): header("Location:" . site_url("admin/appearance/blog"));
      endif;
  ...
  if (route(5)): header("Location:" . site_url("admin/appearance/blog"));
  endif;
...
elseif (route(2) == "language"):
  ...
  // DEBUG: Show current route and access
  echo "<!-- DEBUG: Route(2) = " . route(2) . " -->\n";
  echo "<!-- DEBUG: Route(3) = " . (route(3) ? route(3) : 'NULL') . " -->\n";
  echo "<!-- DEBUG: Access = " . ($access ? 'TRUE' : 'FALSE') . " -->\n";
  ...
  if ($access):
    ...
    elseif (route(3) == "new"):
      echo "<!-- DEBUG: Loading default language file -->\n";
      $defaultFile = 'core/language/default.php';
      echo "<!-- DEBUG: Default file path = " . $defaultFile . " -->\n";
      echo "<!-- DEBUG: File exists = " . (file_exists($defaultFile) ? 'YES' : 'NO') . " -->\n";
      if (file_exists($defaultFile)) {
        include $defaultFile;
        echo "<!-- DEBUG: Default language loaded, languageArray count = " . (isset($languageArray) ? count($languageArray) : 'NOT SET') . " -->\n";
      } else {
        echo "<!-- DEBUG: ERROR - Default language file not found! -->\n";
        $languageArray = array();
      }
    else:
      if (route(3)) {
        echo "<!-- DEBUG: Loading language file for code: " . route(3) . " -->\n";
        ...
        echo "<!-- DEBUG: Language from DB = " . ($language ? $language['language_name'] : 'NOT FOUND') . " -->\n";

        $languageFile = 'core/language/' . route(3) . '.php';
        echo "<!-- DEBUG: Language file path = " . $languageFile . " -->\n";
        echo "<!-- DEBUG: File exists = " . (file_exists($languageFile) ? 'YES' : 'NO') . " -->\n";

        if (file_exists($languageFile)) {
          include $languageFile;
          echo "<!-- DEBUG: Language file loaded, languageArray count = " . (isset($languageArray) ? count($languageArray) : 'NOT SET') . " -->\n";
        } else {
          echo "<!-- DEBUG: ERROR - Language file not found! Setting empty array -->\n";
          $languageArray = array();
        }
      }
    endif;
    if ($_POST && route(3) != "new" && countRow(["table" => "languages", "where" => ["language_code" => route(3)]])):
      ...
      endforeach;
      $html .=  '];';
      $savePath = 'core/language/' . route(3) . '.php';
      echo "<!-- DEBUG: Saving language file to: " . $savePath . " -->\n";
      $result = file_put_contents($savePath, $html);
      echo "<!-- DEBUG: File save result = " . ($result !== false ? 'SUCCESS (' . $result . ' bytes)' : 'FAILED') . " -->\n";
    ...
    elseif (route(3) == "new" && $_POST):
      ...
      if (countRow(["table" => "languages", "where" => ["language_code" => $code]])):
        ...
        if ($insert):
          ...
          endforeach;
          $html .=  '];';
          $savePath = 'core/language/' . $code . '.php';
          echo "<!-- DEBUG: Creating new language file at: " . $savePath . " -->\n";
          $result = file_put_contents($savePath, $html);
          echo "<!-- DEBUG: New file creation result = " . ($result !== false ? 'SUCCESS (' . $result . ' bytes)' : 'FAILED') . " -->\n";
          header("Location:" . site_url("admin/appearance/language/"));
        endif;
      endif;
...
elseif (route(2) == "news"):
  ...
  if (route(5)): header("Location:" . site_url("admin/appearance/news"));
  endif;
