...
elseif ($_POST && $_POST["payment_type"]):
    ...
    if (empty($method_id)) {
        ...
        elseif ($method_id == 9):
            require_once("/core/lib/2checkout/2checkout-php/lib/Twocheckout.php");
        ...
        elseif ($method_id == 12):
            require_once("/core/lib/encdec_paytm.php");
...
';
      ...
      <div class="dimmer-content">
       ...
       <script>
        ...
        elseif ($method_id == 2012):

            $m_orderid = md5(rand(1, 999999));
            $insert = $conn->prepare("INSERT INTO payments SET client_id=:c_id, payment_amount=:amount, payment_privatecode=:code, payment_method=:method, payment_mode=:mode, payment_create_date=:date, payment_ip=:ip, payment_extra=:extra");
            $insert->execute(array("c_id" => $user['client_id'], "amount" => $amount, "code" => $paymentCode, "method" => $method_id, "mode" => "Automatic", "date" => date("Y.m.d H:i:s"), "ip" => GetIP(), "extra" => $m_orderid));

            $m_shop = $extra["m_shop"];

            $m_amount = number_format($amount, 2, '.', '');
            $m_curr = 'USD';
            $m_desc = base64_encode('Payment for Order #' . $m_orderid);
            $m_key = $extra["client_secret"];
...
';
        ...
        elseif ($method_id == 987):
            ...
            $token = $extra['token'];
            $backgroundUrl         = site_url('gbprimepay');;
...
<form action="https://api.gbprimepay.com/v3/qrcode" method="POST" id="pay"> 
  ...
  <input type="hidden" name="backgroundUrl" value="' . $backgroundUrl . '">
        ...
        elseif ($method_id == 31):
            ...
            $MERCHANT_KEY = $extra['public_key'];
            $SALT         =  md5(rand(1, 999999));

            $PAYU_BASE_URL = "https://checkout.flutterwave.com/v3/hosted/pay";
            $famount = $amount;
            $posted = [
                'public_key'              => $MERCHANT_KEY,
                'tx_ref'            => substr(hash('sha256', mt_rand() . microtime()), 0, 20),
                'amount'           => $famount,
                'productinfo'      => 'Add Funds',
                'firstname'        => $user['first_name'],
                'email'            => $user['email'],
                'phone'            => $user['phone'],
                'redirect_url'             => site_url('flutter'),
            ];
            ...
            $hashSequence = "public_key|tx_ref|amount|productinfo|firstname|email|udf1|udf2|udf3|udf4|udf5|udf6|udf7|udf8|udf9|udf10";
    ...
    <form action="https://checkout.flutterwave.com/v3/hosted/pay" method="post" name="flutterform" id="pay">
      ...
      <script type="text/javascript">
        ...
        elseif ($method_id == 207):
            $secret_key =  $extra["secret_key"];
            $category_code = $extra["category_code"];
            $billName = "Deposit - " .  $user['first_name'];
            $billDescription = "Balance recharge - " .  $user['email'];
            $billExternalReferenceNo = time();
            $insert = $conn->prepare("INSERT INTO payments SET client_id=:c_id, payment_amount=:amount, payment_privatecode=:code, payment_method=:method, payment_mode=:mode, payment_create_date=:date, payment_ip=:ip, payment_extra=:extra");
            $insert->execute(array("c_id" => $user['client_id'], "amount" => $amount, "code" => $paymentCode, "method" => $method_id, "mode" => "Auto", "date" => date("Y.m.d H:i:s"), "ip" => GetIP(), "extra" => $billExternalReferenceNo));
            ...
            $payment_data = array(
                ...
                'billReturnUrl' => site_url('toyyib'),
                'billCallbackUrl' => site_url('toyyib'),
...
error_reporting(E_ALL);*/
        ...
        elseif ($method_id == 36):
            ...
            $preferenceData = [
                ...
                'notification_url' => site_url('payment/mercadopago'),
            ...
            if (@$result['init_point']) {
                ...
                $url = $result[($is_demo ? 'sandbox_' : '') . 'init_point'];
                header("Location:" .  $url);
                exit;
            }
            header("Location:" .  site_url('addfunds'));
