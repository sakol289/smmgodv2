...
foreach (glob(__DIR__ . '/include/*.php') as $include) {
  require $include;
}
...
if ($_COOKIE['u_id'] && $_COOKIE['u_login'] && $_COOKIE['u_password']):
  ...
  if (@$_COOKIE['u_password'] == $password):
    ...
    unset($_SESSION['neira_adminlogin']);
    unset($_SESSION);
...
$loader   = new Twig_Loader_Filesystem('core/temp/client/' . THEME);
...
$define = $_SESSION["curid"];


if ($define) {
  $currency = $conn->prepare("SELECT * FROM currency WHERE id=:id");
  $currency->execute(array("id" => $_SESSION["curid"]));
...
foreach (glob(__DIR__ . '/scriptFunc/*.php') as $helper) {
  require $helper;
}
